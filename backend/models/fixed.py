...
class User(SQLModel, table=True):
    ...
    is_admin: bool = Field(default=False, description="Global admin user")
...
class UserCreate(SQLModel):
    ...
    is_admin: bool = Field(default=False, description="grant admin rights")
    is_super_admin: bool = Field(default=False, description="grant super admin rights")
...
class SignupToken(SQLModel, table=True):
    ...
    __table_args__ = {"extend_existing": True}
    token: str = Field(
        ..., primary_key=True, index=True, description="Unique token for signup access"
    )
    ...
    used_by: str | None = Field(
        default=None, description="Username who used this token, if any"
    )
    used_at: datetime.datetime | None = Field(
        default=None, description="When this token was used, if at all"
    )
    notes: str | None = Field(
        default=None, description="Optional notes about who this token is for"
    )
...
class RatingEvent(SQLModel, table=True):
    ...
    poll_label: str = Field(
        index=True, description="Must match a key in your configured polls"
    )
    rating: float = Field(
        description="Discrete or continuous value, interpretation driven by config"
    )
    timestamp: datetime.datetime = Field(
        default_factory=lambda: datetime.datetime.now(datetime.timezone.utc),
