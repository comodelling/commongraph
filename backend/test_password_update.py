...
# Test configuration
TEST_DB_URL = os.getenv("TEST_POSTGRES_DB_URL", "postgresql://test:test@localhost:5432/test_commongraph")
...
def test_password_update_success(test_user):
    ...
    response = client.patch("/users/password", 
        json={
    ...
    old_login_response = client.post("/auth/login", data={
        "username": test_user["username"],
        "password": test_user["password"]
    })
