...
class GraphSchema(SQLModel, table=True):
    ...
    config_hash: str = Field(
        ..., description="Hash of the configuration for change detection"
    )
    ...
    is_active: bool = Field(
        default=True, description="Whether this schema version is active"
    )
...
class SchemaMigration(SQLModel, table=True):
    ...
    changes: Dict[str, Any] = Field(
        sa_column=Column(JSON), description="Detailed changes"
    )
