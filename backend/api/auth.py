...
from backend.models.fixed import UserCreate, UserRead, SignupToken, SignupTokenRead, SignupTokenCreate
...
from backend.config import ALLOW_SIGNUP, SIGNUP_REQUIRES_ADMIN_APPROVAL, SIGNUP_REQUIRES_TOKEN
...
def signup(
    ...
    if SIGNUP_REQUIRES_TOKEN:
        ...
        if signup_token.used_by is not None:
            raise HTTPException(status_code=400, detail="This signup token has already been used")
        ...
        logger.info(f"Signup token {request.signup_token[:8]}... used by {request.user.username}")
